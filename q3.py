...
def repeated_backward_astar(
    ...
    while cur.coord != goal_cell.coord:
        ...
        # Backward planning: search from goal to current on known map.
        path_rev, expanded = compute_path(grid, goal_cell, cur, manhattan, search_id, tie_breaking="max_g")
        expanded_total += len(expanded)
        if path_rev is None:
            return False, executed, expanded_total, replans

        # path_rev is goal..cur; reverse to get cur..goal for execution
